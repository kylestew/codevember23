...
function makeRectangularShape(sizeW, sizeH) {
    ...
    this.makeOriginal = function () {
        for (let a = Math.PI / 4; a < Math.PI * 2.0 + Math.PI / 4; a += (Math.PI * 2.0) / 4) {
            // https://math.stackexchange.com/a/279209/673569
            let ax = 0.5 * this.sizeW * Math.sign(Math.cos(a))
            let ay = 0.5 * this.sizeH * Math.sign(Math.sin(a))

            let axn = 0.5 * this.sizeW * Math.sign(Math.cos(a + Math.PI / 2))
            let ayn = 0.5 * this.sizeH * Math.sign(Math.sin(a + Math.PI / 2))

            for (let inter = 0; inter < 1; inter += 0.2) {
    ...
    this.scaleRotateTranslate = function (scale, rotateRadians, translateX, translateY) {
        this.circles = []
...
const sketch = () => {
    return async ({ context, width, height }) => {
        // load font file from current directory
        const response = await fetch('sketches/assets/FiraSansOT-Medium.otf')
        const buffer = await response.arrayBuffer()
        const font = opentype.parse(buffer)
        console.log(font)

        // font.draw(context, 'Hello, World!', 10, 100, 72)
        font.drawPoints(context, 'H', 10, 100, 72)

        // TODO: will need to evenly space points along paths
        const path = font.getPath('H', 10, 100, 72)
        console.log(path.commands)
        // console.log(font.getPath('H', 10, 100, 72))

        // so this makes a shape and turns it into boundaries of circles
        // const rect = new makeRectangularShape(100, 100)
        // rect.makeOriginal()
        // console.log(rect.original)
        // draw a circle at each point defined in rect.original
        // console.log(rect.original.length)
        // rect.original.forEach((c) => {
        //     context.beginPath()
        //     context.arc(c.x + width / 2, c.y + height / 2, c.r, 0, Math.PI * 2, false)
        //     context.strokeWeight = 1.0
...
function makeCirclePacker(wid, hei, gridDivs, gridPadding) {
    ...
    this.getGridTilesAround = function (x, y, r) {
        const tl = [floor((x - r - this.pad) / this.gridSizeX), floor((y - r - this.pad) / this.gridSizeY)]

        const br = [floor((x + r + this.pad) / this.gridSizeX), floor((y + r + this.pad) / this.gridSizeY)]
    ...
    this.tryToAddCircle = function (x, y, minRadius = 3, maxRadius = 200, actuallyAdd = true) {
        let c1 = {
            x,
            y,
            r: minRadius,
...
function makeTriangularShape(size) {
    ...
    this.sizeParam = [random(0.25, 1.5), random(0.25, 1.5), random(0.25, 1.5)]
    ...
    this.makeOriginal = function () {
        ...
        for (let a = 0; a < TAU; a += TAU / 3) {
            ...
            for (let inter = 0; inter < 1; inter += 0.05) {
                let xi = ax * inter + axn * (1 - inter)
                let yi = ay * inter + ayn * (1 - inter)
